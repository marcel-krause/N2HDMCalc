...
#include "looptools.h"
    ...
    double precision H2toTTBarTree, H2toTTBarCT, H2toTTBarReal, treeLevelTemp, realCorrectionsTemp
    double complex H2toTTBarVC, H2toTTBarTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(H2toTTBarVC()) + H2toTTBarCT(n)
                else
                    UVChecks(1) = DBLE(H2toTTBarVC() + H2toTTBarTad()) + &
                                & H2toTTBarCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(H2toTTBarVC()) + H2toTTBarCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(H2toTTBarVC() + H2toTTBarTad()) + &
                                    & H2toTTBarCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MH2**4 + MT**4 + MT**4 - 2D0*MH2**2*MT**2 - 2D0*MH2**2*MT**2 &
                        &  - 2D0*MT**2*MT**2 )/(16D0*PI*MH2**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(H2toTTBarVC()) + &
                                & 2D0*H2toTTBarCT(n) + &
                                & H2toTTBarReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(H2toTTBarVC() + &
                                & H2toTTBarTad()) + 2D0*H2toTTBarCT(n) + &
                                & H2toTTBarReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H2toTTBarVC()) + &
                                    & 2D0*H2toTTBarCT(n) + H2toTTBarReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H2toTTBarVC() + &
                                    & H2toTTBarTad()) + 2D0*H2toTTBarCT(n) + &
                                    & H2toTTBarReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", H2toTTBarReal()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*H2toTTBarTree()

                ! Calculate the NLO ingredients
                treeLevelTemp = H2toTTBarTree()
                vertexCorrectionsTemp = H2toTTBarVC()
                vertexTadpolesTemp = H2toTTBarTad()
                realCorrectionsTemp = H2toTTBarReal()
