...
#include "looptools.h"
    ...
    double precision H2toTauTauBarTree, H2toTauTauBarCT, H2toTauTauBarReal, treeLevelTemp, realCorrectionsTemp
    double complex H2toTauTauBarVC, H2toTauTauBarTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(H2toTauTauBarVC()) + H2toTauTauBarCT(n)
                else
                    UVChecks(1) = DBLE(H2toTauTauBarVC() + H2toTauTauBarTad()) + &
                                & H2toTauTauBarCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(H2toTauTauBarVC()) + H2toTauTauBarCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(H2toTauTauBarVC() + H2toTauTauBarTad()) + &
                                    & H2toTauTauBarCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MH2**4 + ML**4 + ML**4 - 2D0*MH2**2*ML**2 - 2D0*MH2**2*ML**2 &
                        &  - 2D0*ML**2*ML**2 )/(16D0*PI*MH2**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(H2toTauTauBarVC()) + &
                                & 2D0*H2toTauTauBarCT(n) + &
                                & H2toTauTauBarReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(H2toTauTauBarVC() + &
                                & H2toTauTauBarTad()) + 2D0*H2toTauTauBarCT(n) + &
                                & H2toTauTauBarReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H2toTauTauBarVC()) + &
                                    & 2D0*H2toTauTauBarCT(n) + H2toTauTauBarReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H2toTauTauBarVC() + &
                                    & H2toTauTauBarTad()) + 2D0*H2toTauTauBarCT(n) + &
                                    & H2toTauTauBarReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", H2toTauTauBarReal()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*H2toTauTauBarTree()

                ! Calculate the NLO ingredients
                treeLevelTemp = H2toTauTauBarTree()
                vertexCorrectionsTemp = H2toTauTauBarVC()
                vertexTadpolesTemp = H2toTauTauBarTad()
                realCorrectionsTemp = H2toTauTauBarReal()
