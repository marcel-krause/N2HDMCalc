...
#include "looptools.h"
    ...
    double precision HptoH1WpTree, HptoH1WpCT, HptoH1WpReal, treeLevelTemp, realCorrectionsTemp
    double complex HptoH1WpVC, HptoH1WpTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(HptoH1WpVC()) + HptoH1WpCT(n)
                else
                    UVChecks(1) = DBLE(HptoH1WpVC() + HptoH1WpTad()) + &
                                & HptoH1WpCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(HptoH1WpVC()) + HptoH1WpCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(HptoH1WpVC() + HptoH1WpTad()) + &
                                    & HptoH1WpCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MHp**4 + MH1**4 + MW**4 - 2D0*MHp**2*MH1**2 - 2D0*MHp**2*MW**2 &
                        &  - 2D0*MH1**2*MW**2 )/(16D0*PI*MHp**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(HptoH1WpVC()) + &
                                & 2D0*HptoH1WpCT(n) + &
                                & HptoH1WpReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(HptoH1WpVC() + &
                                & HptoH1WpTad()) + 2D0*HptoH1WpCT(n) + &
                                & HptoH1WpReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(HptoH1WpVC()) + &
                                    & 2D0*HptoH1WpCT(n) + HptoH1WpReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(HptoH1WpVC() + &
                                    & HptoH1WpTad()) + 2D0*HptoH1WpCT(n) + &
                                    & HptoH1WpReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", HptoH1WpReal()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*HptoH1WpTree()

                ! Calculate the NLO ingredients
                treeLevelTemp = HptoH1WpTree()
                vertexCorrectionsTemp = HptoH1WpVC()
                vertexTadpolesTemp = HptoH1WpTad()
                realCorrectionsTemp = HptoH1WpReal()
