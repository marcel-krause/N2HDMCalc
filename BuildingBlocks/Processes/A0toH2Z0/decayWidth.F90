...
#include "looptools.h"
    ...
    double precision A0toH2Z0Tree, A0toH2Z0CT, A0toH2Z0Real, treeLevelTemp, realCorrectionsTemp
    double complex A0toH2Z0VC, A0toH2Z0Tad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(A0toH2Z0VC()) + A0toH2Z0CT(n)
                else
                    UVChecks(1) = DBLE(A0toH2Z0VC() + A0toH2Z0Tad()) + &
                                & A0toH2Z0CT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(A0toH2Z0VC()) + A0toH2Z0CT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(A0toH2Z0VC() + A0toH2Z0Tad()) + &
                                    & A0toH2Z0CT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MA0**4 + MH2**4 + MZ**4 - 2D0*MA0**2*MH2**2 - 2D0*MA0**2*MZ**2 &
                        &  - 2D0*MH2**2*MZ**2 )/(16D0*PI*MA0**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(A0toH2Z0VC()) + &
                                & 2D0*A0toH2Z0CT(n) + &
                                & A0toH2Z0Real() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(A0toH2Z0VC() + &
                                & A0toH2Z0Tad()) + 2D0*A0toH2Z0CT(n) + &
                                & A0toH2Z0Real() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(A0toH2Z0VC()) + &
                                    & 2D0*A0toH2Z0CT(n) + A0toH2Z0Real() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(A0toH2Z0VC() + &
                                    & A0toH2Z0Tad()) + 2D0*A0toH2Z0CT(n) + &
                                    & A0toH2Z0Real() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", A0toH2Z0Real()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*A0toH2Z0Tree()

                ! Calculate the NLO ingredients
                treeLevelTemp = A0toH2Z0Tree()
                vertexCorrectionsTemp = A0toH2Z0VC()
                vertexTadpolesTemp = A0toH2Z0Tad()
                realCorrectionsTemp = A0toH2Z0Real()
