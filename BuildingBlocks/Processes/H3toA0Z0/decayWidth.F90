...
#include "looptools.h"
    ...
    double precision H3toA0Z0Tree, H3toA0Z0CT, H3toA0Z0Real, treeLevelTemp, realCorrectionsTemp
    double complex H3toA0Z0VC, H3toA0Z0Tad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(H3toA0Z0VC()) + H3toA0Z0CT(n)
                else
                    UVChecks(1) = DBLE(H3toA0Z0VC() + H3toA0Z0Tad()) + &
                                & H3toA0Z0CT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(H3toA0Z0VC()) + H3toA0Z0CT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(H3toA0Z0VC() + H3toA0Z0Tad()) + &
                                    & H3toA0Z0CT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MH3**4 + MA0**4 + MZ**4 - 2D0*MH3**2*MA0**2 - 2D0*MH3**2*MZ**2 &
                        &  - 2D0*MA0**2*MZ**2 )/(16D0*PI*MH3**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(H3toA0Z0VC()) + &
                                & 2D0*H3toA0Z0CT(n) + &
                                & H3toA0Z0Real() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(H3toA0Z0VC() + &
                                & H3toA0Z0Tad()) + 2D0*H3toA0Z0CT(n) + &
                                & H3toA0Z0Real() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H3toA0Z0VC()) + &
                                    & 2D0*H3toA0Z0CT(n) + H3toA0Z0Real() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H3toA0Z0VC() + &
                                    & H3toA0Z0Tad()) + 2D0*H3toA0Z0CT(n) + &
                                    & H3toA0Z0Real() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", H3toA0Z0Real()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*H3toA0Z0Tree()

                ! Calculate the NLO ingredients
                treeLevelTemp = H3toA0Z0Tree()
                vertexCorrectionsTemp = H3toA0Z0VC()
                vertexTadpolesTemp = H3toA0Z0Tad()
                realCorrectionsTemp = H3toA0Z0Real()
