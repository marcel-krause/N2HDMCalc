...
#include "looptools.h"
    ...
    double precision HptoH2WpTree, HptoH2WpCT, HptoH2WpReal, treeLevelTemp, realCorrectionsTemp
    double complex HptoH2WpVC, HptoH2WpTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(HptoH2WpVC()) + HptoH2WpCT(n)
                else
                    UVChecks(1) = DBLE(HptoH2WpVC() + HptoH2WpTad()) + &
                                & HptoH2WpCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(HptoH2WpVC()) + HptoH2WpCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(HptoH2WpVC() + HptoH2WpTad()) + &
                                    & HptoH2WpCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MHp**4 + MH2**4 + MW**4 - 2D0*MHp**2*MH2**2 - 2D0*MHp**2*MW**2 &
                        &  - 2D0*MH2**2*MW**2 )/(16D0*PI*MHp**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(HptoH2WpVC()) + &
                                & 2D0*HptoH2WpCT(n) + &
                                & HptoH2WpReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(HptoH2WpVC() + &
                                & HptoH2WpTad()) + 2D0*HptoH2WpCT(n) + &
                                & HptoH2WpReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(HptoH2WpVC()) + &
                                    & 2D0*HptoH2WpCT(n) + HptoH2WpReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(HptoH2WpVC() + &
                                    & HptoH2WpTad()) + 2D0*HptoH2WpCT(n) + &
                                    & HptoH2WpReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", HptoH2WpReal()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*HptoH2WpTree()

                ! Calculate the NLO ingredients
                treeLevelTemp = HptoH2WpTree()
                vertexCorrectionsTemp = HptoH2WpVC()
                vertexTadpolesTemp = HptoH2WpTad()
                realCorrectionsTemp = HptoH2WpReal()
