...
#include "looptools.h"
    ...
    double precision H1toElElBarTree, H1toElElBarCT, H1toElElBarReal, treeLevelTemp, realCorrectionsTemp
    double complex H1toElElBarVC, H1toElElBarTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(H1toElElBarVC()) + H1toElElBarCT(n)
                else
                    UVChecks(1) = DBLE(H1toElElBarVC() + H1toElElBarTad()) + &
                                & H1toElElBarCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(H1toElElBarVC()) + H1toElElBarCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(H1toElElBarVC() + H1toElElBarTad()) + &
                                    & H1toElElBarCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MH1**4 + ME**4 + ME**4 - 2D0*MH1**2*ME**2 - 2D0*MH1**2*ME**2 &
                        &  - 2D0*ME**2*ME**2 )/(16D0*PI*MH1**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(H1toElElBarVC()) + &
                                & 2D0*H1toElElBarCT(n) + &
                                & H1toElElBarReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(H1toElElBarVC() + &
                                & H1toElElBarTad()) + 2D0*H1toElElBarCT(n) + &
                                & H1toElElBarReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H1toElElBarVC()) + &
                                    & 2D0*H1toElElBarCT(n) + H1toElElBarReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H1toElElBarVC() + &
                                    & H1toElElBarTad()) + 2D0*H1toElElBarCT(n) + &
                                    & H1toElElBarReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", H1toElElBarReal()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*H1toElElBarTree()

                ! Calculate the NLO ingredients
                treeLevelTemp = H1toElElBarTree()
                vertexCorrectionsTemp = H1toElElBarVC()
                vertexTadpolesTemp = H1toElElBarTad()
                realCorrectionsTemp = H1toElElBarReal()
