...
#include "looptools.h"
    ...
    double precision H3toSSBarTree, H3toSSBarCT, H3toSSBarReal, treeLevelTemp, realCorrectionsTemp
    double complex H3toSSBarVC, H3toSSBarTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(H3toSSBarVC()) + H3toSSBarCT(n)
                else
                    UVChecks(1) = DBLE(H3toSSBarVC() + H3toSSBarTad()) + &
                                & H3toSSBarCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(H3toSSBarVC()) + H3toSSBarCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(H3toSSBarVC() + H3toSSBarTad()) + &
                                    & H3toSSBarCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MH3**4 + MS**4 + MS**4 - 2D0*MH3**2*MS**2 - 2D0*MH3**2*MS**2 &
                        &  - 2D0*MS**2*MS**2 )/(16D0*PI*MH3**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(H3toSSBarVC()) + &
                                & 2D0*H3toSSBarCT(n) + &
                                & H3toSSBarReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(H3toSSBarVC() + &
                                & H3toSSBarTad()) + 2D0*H3toSSBarCT(n) + &
                                & H3toSSBarReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H3toSSBarVC()) + &
                                    & 2D0*H3toSSBarCT(n) + H3toSSBarReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H3toSSBarVC() + &
                                    & H3toSSBarTad()) + 2D0*H3toSSBarCT(n) + &
                                    & H3toSSBarReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", H3toSSBarReal()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*H3toSSBarTree()

                ! Calculate the NLO ingredients
                treeLevelTemp = H3toSSBarTree()
                vertexCorrectionsTemp = H3toSSBarVC()
                vertexTadpolesTemp = H3toSSBarTad()
                realCorrectionsTemp = H3toSSBarReal()
