...
#include "looptools.h"
    ...
    double precision H3toH1H2Tree, H3toH1H2CT, H3toH1H2Real, treeLevelTemp, realCorrectionsTemp
    double complex H3toH1H2VC, H3toH1H2Tad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(H3toH1H2VC()) + H3toH1H2CT(n)
                else
                    UVChecks(1) = DBLE(H3toH1H2VC() + H3toH1H2Tad()) + &
                                & H3toH1H2CT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(H3toH1H2VC()) + H3toH1H2CT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(H3toH1H2VC() + H3toH1H2Tad()) + &
                                    & H3toH1H2CT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MH3**4 + MH1**4 + MH2**4 - 2D0*MH3**2*MH1**2 - 2D0*MH3**2*MH2**2 &
                        &  - 2D0*MH1**2*MH2**2 )/(16D0*PI*MH3**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(H3toH1H2VC()) + &
                                & 2D0*H3toH1H2CT(n) + &
                                & H3toH1H2Real() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(H3toH1H2VC() + &
                                & H3toH1H2Tad()) + 2D0*H3toH1H2CT(n) + &
                                & H3toH1H2Real() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H3toH1H2VC()) + &
                                    & 2D0*H3toH1H2CT(n) + H3toH1H2Real() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H3toH1H2VC() + &
                                    & H3toH1H2Tad()) + 2D0*H3toH1H2CT(n) + &
                                    & H3toH1H2Real() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", H3toH1H2Real()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*H3toH1H2Tree()

                ! Calculate the NLO ingredients
                treeLevelTemp = H3toH1H2Tree()
                vertexCorrectionsTemp = H3toH1H2VC()
                vertexTadpolesTemp = H3toH1H2Tad()
                realCorrectionsTemp = H3toH1H2Real()
