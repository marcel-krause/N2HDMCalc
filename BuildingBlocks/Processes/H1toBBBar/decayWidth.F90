...
#include "looptools.h"
    ...
    double precision H1toBBBarTree, H1toBBBarCT, H1toBBBarReal, treeLevelTemp, realCorrectionsTemp
    double complex H1toBBBarVC, H1toBBBarTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(H1toBBBarVC()) + H1toBBBarCT(n)
                else
                    UVChecks(1) = DBLE(H1toBBBarVC() + H1toBBBarTad()) + &
                                & H1toBBBarCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(H1toBBBarVC()) + H1toBBBarCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(H1toBBBarVC() + H1toBBBarTad()) + &
                                    & H1toBBBarCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MH1**4 + MB**4 + MB**4 - 2D0*MH1**2*MB**2 - 2D0*MH1**2*MB**2 &
                        &  - 2D0*MB**2*MB**2 )/(16D0*PI*MH1**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(H1toBBBarVC()) + &
                                & 2D0*H1toBBBarCT(n) + &
                                & H1toBBBarReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(H1toBBBarVC() + &
                                & H1toBBBarTad()) + 2D0*H1toBBBarCT(n) + &
                                & H1toBBBarReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H1toBBBarVC()) + &
                                    & 2D0*H1toBBBarCT(n) + H1toBBBarReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H1toBBBarVC() + &
                                    & H1toBBBarTad()) + 2D0*H1toBBBarCT(n) + &
                                    & H1toBBBarReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", H1toBBBarReal()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*H1toBBBarTree()

                ! Calculate the NLO ingredients
                treeLevelTemp = H1toBBBarTree()
                vertexCorrectionsTemp = H1toBBBarVC()
                vertexTadpolesTemp = H1toBBBarTad()
                realCorrectionsTemp = H1toBBBarReal()
