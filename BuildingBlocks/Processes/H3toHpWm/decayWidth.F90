...
#include "looptools.h"
    ...
    double precision H3toHpWmTree, H3toHpWmCT, H3toHpWmReal, treeLevelTemp, realCorrectionsTemp
    double complex H3toHpWmVC, H3toHpWmTad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(H3toHpWmVC()) + H3toHpWmCT(n)
                else
                    UVChecks(1) = DBLE(H3toHpWmVC() + H3toHpWmTad()) + &
                                & H3toHpWmCT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(H3toHpWmVC()) + H3toHpWmCT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(H3toHpWmVC() + H3toHpWmTad()) + &
                                    & H3toHpWmCT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/1D0 * DSQRT(MH3**4 + MHp**4 + MW**4 - 2D0*MH3**2*MHp**2 - 2D0*MH3**2*MW**2 &
                        &  - 2D0*MHp**2*MW**2 )/(16D0*PI*MH3**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(H3toHpWmVC()) + &
                                & 2D0*H3toHpWmCT(n) + &
                                & H3toHpWmReal() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(H3toHpWmVC() + &
                                & H3toHpWmTad()) + 2D0*H3toHpWmCT(n) + &
                                & H3toHpWmReal() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H3toHpWmVC()) + &
                                    & 2D0*H3toHpWmCT(n) + H3toHpWmReal() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H3toHpWmVC() + &
                                    & H3toHpWmTad()) + 2D0*H3toHpWmCT(n) + &
                                    & H3toHpWmReal() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", H3toHpWmReal()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*H3toHpWmTree()

                ! Calculate the NLO ingredients
                treeLevelTemp = H3toHpWmTree()
                vertexCorrectionsTemp = H3toHpWmVC()
                vertexTadpolesTemp = H3toHpWmTad()
                realCorrectionsTemp = H3toHpWmReal()
