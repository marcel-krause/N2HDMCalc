...
#include "looptools.h"
    ...
    double precision H3toH1H1Tree, H3toH1H1CT, H3toH1H1Real, treeLevelTemp, realCorrectionsTemp
    double complex H3toH1H1VC, H3toH1H1Tad, vertexCorrectionsTemp, vertexTadpolesTemp
        ...
        call ltini
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    UVChecks(1) = DBLE(H3toH1H1VC()) + H3toH1H1CT(n)
                else
                    UVChecks(1) = DBLE(H3toH1H1VC() + H3toH1H1Tad()) + &
                                & H3toH1H1CT(n)
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        UVChecks(m) = ( DBLE(H3toH1H1VC()) + H3toH1H1CT(n) - &
                                    & UVChecks(1) )/UVChecks(1)
                    else
                        UVChecks(m) = ( DBLE(H3toH1H1VC() + H3toH1H1Tad()) + &
                                    & H3toH1H1CT(n) - UVChecks(1) &
                                    & )/UVChecks(1)
                    end if
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
        ...
        call ltini
            ...
            prefactor = 1D0/2D0 * DSQRT(MH3**4 + MH1**4 + MH1**4 - 2D0*MH3**2*MH1**2 - 2D0*MH3**2*MH1**2 &
                        &  - 2D0*MH1**2*MH1**2 )/(16D0*PI*MH3**3)
            ...
            do n = 1, maxNumberSchemes, 1
                ...
                ! Schemes 1, 2, and 9 are without tadpoles
                if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                    IRChecks(1) = prefactor*( 2D0*DBLE(H3toH1H1VC()) + &
                                & 2D0*H3toH1H1CT(n) + &
                                & H3toH1H1Real() )
                else
                    IRChecks(1) = prefactor*( 2D0*DBLE(H3toH1H1VC() + &
                                & H3toH1H1Tad()) + 2D0*H3toH1H1CT(n) + &
                                & H3toH1H1Real() )
                end if
                do m = 2, 10, 1
                    call clearcache
                    ...
                    ! Schemes 1, 2, and 9 are without tadpoles
                    if ((n == 1) .OR. (n == 2) .OR. (n == 9)) then
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H3toH1H1VC()) + &
                                    & 2D0*H3toH1H1CT(n) + H3toH1H1Real() ) &
                                    & - IRChecks(1))/IRChecks(1)
                    else
                        IRChecks(m) = ( prefactor*( 2D0*DBLE(H3toH1H1VC() + &
                                    & H3toH1H1Tad()) + 2D0*H3toH1H1CT(n) + &
                                    & H3toH1H1Real() ) - IRChecks(1))/IRChecks(1)
                    end if
                    print *, "Lambda: ", getlambda(), ", Difference from lambda=1: ", IRChecks(m)
                    print *, "Real: ", H3toH1H1Real()
...
CS1S1S3S3f3322 = 0.5D0*(-1.D0*RR31*(-2.D0*CB*(Lam4 + Lam5)*RR32*SB + 2.D0*RR31*(CB2*Lam3 + Lam1*SB2)) - 1.D0*RR32*(-2.D0*CB*(Lam4&
  & + Lam5)*RR31*SB + 2.D0*RR32*(CB2*Lam2 + Lam3*SB2)) - 2.D0*(CB2*Lam8 + Lam7*SB2)*DBLE(RR33**INT(2.D0)))
                ...
                call clearcache
                treeLevelWidth = prefactor*H3toH1H1Tree()

                ! Calculate the NLO ingredients
                treeLevelTemp = H3toH1H1Tree()
                vertexCorrectionsTemp = H3toH1H1VC()
                vertexTadpolesTemp = H3toH1H1Tad()
                realCorrectionsTemp = H3toH1H1Real()
