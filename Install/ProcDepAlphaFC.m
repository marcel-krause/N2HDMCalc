...
ampList=FileNames["*.txt",{FileNameJoin[{dir, "..", "Temp", "procDepAlpha"}]}];
exportFortran = "";
exportFortran = exportFortran <> "double complex function HHtoTauPTauMProcDepVC()\n";
...
For[i=1,i<=Length[ampList],i++,
	...
	export = Simplify[(oneloop/.{StandardMatrixElement[_]:>1})/(-EL*ML*Yuk5/(2*MW*SW))];
...
exportFortran=exportFortran<>"HHtoTauPTauMProcDepVC = totalAmplitude\n";
exportFortran=exportFortran<>"end function HHtoTauPTauMProcDepVC";
filename = FileNameJoin[{dir, "..", "BuildingBlocks", "ProcessDependentScheme", "HHtoTauPTauMProcDepVC.txt"}];
