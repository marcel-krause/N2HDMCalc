...
ImportFile = FileNameJoin[{dir, "..", "BuildingBlocks", "SelfEnergies", targetDir, "SelfEnergyTauTauWeakFA.txt"}]; 
...
For[n=1, n<=Length[fermionParts], n++,
	...
	exportScalar=0;
	exportFortran = "";
	exportFortran = exportFortran <> "double complex function SelfTauTau" <> fermionParts[[n]] <> "Weak" <> ampVersion <> "(x)\n";
	...
	exportFortran = exportFortran <> " double complex :: amplitudes("<>ToString[Length[SelfTauTauWeak]]<>")\n\n";
	exportFortranDeriv = "";
	exportFortranDeriv = exportFortranDeriv <> "double complex function DSelfTauTau" <> fermionParts[[n]] <> "Weak(x)\n";
	...
	exportFortranDeriv = exportFortranDeriv <> " double complex :: amplitudes("<>ToString[Length[SelfTauTauWeak]]<>")\n\n";
	For[i = 1, i <= Length[SelfTauTauWeak], i++, 
		Print["Calculating sub-amplitude number "<> ToString[ampCounter] <>" ..."];
		ampCounter++;
		tracer = (((SelfTauTauWeak[[i]]/.truncated)/.{PropagatorDenominator[0,0]:>PropagatorDenominator[0,\[Mu]]})/.{DiracTrace -> Tr}); 
		If[tracer == 0, exportFortran=exportFortran<>" amplitudes("<>ToString[i]<>") = 0.D0"<>"\n\n"; exportFortranDeriv=exportFortranDeriv<>" amplitudes("<>ToString[i]<>") = 0.D0"<>"\n\n"; 
	...
	exportFortran=exportFortran<>" do j=1,"<>ToString[Length[SelfTauTauWeak]]<>"\n";
	exportFortran=exportFortran<>"  totalAmplitude = totalAmplitude + amplitudes(j)\n";
	exportFortran=exportFortran<>" end do\n";
	exportFortran=exportFortran<>" SelfTauTau" <> fermionParts[[n]] <> "Weak" <> ampVersion <> " = totalAmplitude\n";
	exportFortran=exportFortran<>"end function SelfTauTau" <> fermionParts[[n]] <> "Weak" <> ampVersion <> "\n\n";
	exportFortranDeriv=exportFortranDeriv<>"  totalAmplitude = (0D0,0D0)\n";
	exportFortranDeriv=exportFortranDeriv<>" do j=1,"<>ToString[Length[SelfTauTauWeak]]<>"\n";
	...
	exportFortranDeriv=exportFortranDeriv<>" DSelfTauTau" <> fermionParts[[n]] <> "Weak = totalAmplitude\n";
	exportFortranDeriv=exportFortranDeriv<>"end function DSelfTauTau" <> fermionParts[[n]] <> "Weak\n\n";
	filename = FileNameJoin[{dir, "..", "BuildingBlocks", "SelfEnergies", targetDir, "SelfTauTau" <> fermionParts[[n]] <> "Weak.txt"}];
	...
	filenameDeriv = FileNameJoin[{dir, "..", "BuildingBlocks", "SelfEnergiesDerivatives", "DSelfTauTau" <> fermionParts[[n]] <> "Weak.txt"}];
